...
2) Wallet payment
- Process payment in the wallet; obtain one or more transaction hashes (`txhashes`) from success.
3) Create advertisement
- POST `/api/ads/protected/create` with `{ title, imageUrl, rooms, minutes, txhashes }` and header `x-user-fid`.
